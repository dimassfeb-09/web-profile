...
import type { HomeData } from '../repositories/home.repository';
import { cached, revalidateTag } from '$lib/cache';

export class HomeService {
	private static getCachedHomeData = () =>
		cached('home_data', () => HomeRepository.findFirst(), { ttl: 3600, tags: ['home'] });

	static async getHomeData(bypassCache = false) {
		try {
			const home = bypassCache ? await HomeRepository.findFirst() : await this.getCachedHomeData();
